...
def retrieve_yolo_detections(yolo_result, anchors, min_probability, nms_thresh, image_input_size, image_size, labels_dict):
    ...
    correct_yolo_boxes(
        boxes, image_size[1], image_size[0], image_input_size[0], image_input_size[1])
...
def draw_boxes(image, box_points, draw_box, label, percentage_probability, color):
    ...
    if label is not None or percentage_probability is not None:
        cv2.putText(image, label, (xmin, ymin - 13),
                    cv2.FONT_HERSHEY_SIMPLEX, 1e-3 * image.shape[0], (255, 0, 0), 2)
        cv2.putText(image, label, (xmin, ymin - 13), cv2.FONT_HERSHEY_SIMPLEX,
                    1e-3 * image.shape[0], (255, 255, 255), 1)
